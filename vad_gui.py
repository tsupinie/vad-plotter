...
class VADWidget(QtGui.QWidget):
    def __init__(self, parent=None):
        ...
        self._pix_wid, self._pix_hgt = 400, 400

        self._initWidget()

    def _pix2uv(self, pix_x, pix_y):
        u_pt = self._min_u + (self._max_u - self._min_u) * float(pix_x) / self._pix_wid
        v_pt = self._max_v - (self._max_v - self._min_v) * float(pix_y) / self._pix_hgt
        return u_pt, v_pt

    def _uv2Pix(self, u_pt, v_pt):
        pix_x = (u_pt - self._min_u) / (self._max_u - self._min_u) * self._pix_wid
        pix_y = (self._max_v - v_pt) / (self._max_v - self._min_v) * self._pix_hgt
    ...
    def _drawBackground(self):
        ...
        pen.setWidth(1.5)
        pen.setBrush(QtGui.QColor('#888888'))
